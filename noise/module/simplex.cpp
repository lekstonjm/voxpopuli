 ...
 * http://webstaff.itn.liu.se/~stegu/Simplex/Simplex.java:
...
Simplex::Simplex(float frequency,
        float amplitude,
        float lacunarity,
        float persistence,
        size_t octaves,
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float Simplex::fractal(size_t octaves, float x) const {
    float output    = 0.f;
    float denom     = 0.f;
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float Simplex::fractal(size_t octaves, float x, float y) const {
    float output = 0.f;
    float denom  = 0.f;
    float frequency = mFrequency;
    float amplitude = mAmplitude;
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float Simplex::fractal(size_t octaves, float x, float y, float z) const {
    float output = 0.f;
    float denom  = 0.f;
    float frequency = mFrequency;
    float amplitude = mAmplitude;
...
double Simplex::GetValue (double x, double y, double z) const {
    if (isFractal()) {
        return fractal(mOctaves, x,y,z);
    } else {
        return noise(x,y,z);
    }
}
